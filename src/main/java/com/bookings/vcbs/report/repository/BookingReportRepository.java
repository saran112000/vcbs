...
public interface BookingReportRepository extends JpaRepository<Bookings, Long> {
	...
	@Query(value = """
		    SELECT b.booking_id, b.booking_date, b.booking_by, e.emp_name AS slotBookedBy, 
		           e.desig_id, ed.designation AS slotBookedByDesignation,
		           b.room_id, rd.room_no, rd.room_type_id, rt.room_type, 
		           b.guest_name, b.subject, b.status, 
		           s.bookedSlots, s.remarks, 
		           ce.emp_name AS cancelledBy,
		           ced.designation AS cancelledByDesignation -- Added this line
		    FROM bookings b
		    ...
		    INNER JOIN (
		        SELECT s.booking_id, 
		               ...
		               MAX(s.cancelled_by) AS cancelledById,
		               GROUP_CONCAT(s.slot_time ORDER BY s.slot_time) AS bookedSlots
		        FROM bookings_slot_details s 
		        WHERE s.is_active = :isActive 
		        GROUP BY s.booking_id
		    ) s ON s.booking_id = b.booking_id
		    LEFT JOIN employee ce ON ce.emp_id = s.cancelledById
		    LEFT JOIN employee_desig ced ON ced.desig_id = ce.desig_id -- Join for canceller designation
		    WHERE b.status = :status AND (b.booking_date BETWEEN :fromDate AND :toDate)
		""", nativeQuery = true)
		List<BookingDetailProjection> getRoomBookedList(@Param("status") String status, @Param("isActive") int isActive, LocalDate fromDate, LocalDate toDate);
}
